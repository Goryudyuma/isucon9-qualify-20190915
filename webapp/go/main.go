...
func getNewCategoryItems(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		inQuery, inArgs, err = sqlx.In(
			"SELECT * FROM (SELECT * FROM `items` WHERE `status` = ? AND category_id IN (?) AND (`created_at` < ?  OR (`created_at` <= ? AND `id` < ?)) " +
				"UNION " +
				"SELECT * FROM `items` WHERE `status` = ? AND category_id IN (?) AND (`created_at` < ?  OR (`created_at` <= ? AND `id` < ?))) as a ORDER BY `created_at` DESC, `id` DESC LIMIT ?",
			ItemStatusOnSale,
			categoryIDs,
			time.Unix(createdAt, 0),
			time.Unix(createdAt, 0),
